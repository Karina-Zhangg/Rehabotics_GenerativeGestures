    ...
    def do_POST(self):
        ...
        if self.path == '/update':
            ...
            for servo_id, value in positions.items():
                ...
                servo_key = f"servo_{servo_id}"  # Convert back to servo_1 format for limits lookup
                min_pos = gestures["servo_limits"][servo_key]["min"]
                max_pos = gestures["servo_limits"][servo_key]["max"]
